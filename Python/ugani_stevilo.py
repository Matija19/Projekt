...
# =====================================================================@009833=
# 1. podnaloga
# Sestavite funkcijo `pridobi_in_primerjaj(x)`, ki uporabnika vpraša za vnos naravnega
# števila. Nato vneseno število primerja s številom `x`. Če sta števili enaki,
# vrne `True`, sicer pa `False`.
# =============================================================================

# =====================================================================@009834=
# 2. podnaloga
# Sestavite funkcijo `ugibanje(x)`, ki uporabnika vpraša za vnos naravnega
# števila. Nato vneseno število primerja s številom `x`. Če sta števili enaki,
# program izpiše "Bravo!" in se zaključi, sicer pa se ugibanje nadaljuje.
# =============================================================================

# =====================================================================@009835=
# 3. podnaloga
# Ustvarite novo datoteko `ugibaj.py`. Vanjo najprej skopirajte
# funkciji iz prejšnjih dveh delov. Na koncu dodajte vrstico:
# 
#       ugibanje(7)
# 
# Program nato poženite z ukazno vrstico.
# V urejevalniku kot rešitev zapišite le `pass`.
# =============================================================================
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            with Check.input(['7']):
                Check.equal('pridobi_in_primerjaj(7)', True)
            with Check.input(['7']):
                Check.equal('pridobi_in_primerjaj(6)', False)
            with Check.input(['7']):
                Check.equal('pridobi_in_primerjaj(5)', False)
            
            with Check.input(['0']):
                Check.equal('pridobi_in_primerjaj(0)', True)
            with Check.input(['0']):
                Check.equal('pridobi_in_primerjaj(1)', False)
            with Check.input(['0']):    
                Check.equal('pridobi_in_primerjaj(2)', False)
            with Check.input(['0']):    
                Check.equal('pridobi_in_primerjaj(100)', False)
        except:
    ...
    if Check.part():
        try:
            for i in range(10):
                with Check.input([str(i)]):
                    Check.output('ugibanje({})'.format(i), ['Bravo!'], use_globals=True)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
